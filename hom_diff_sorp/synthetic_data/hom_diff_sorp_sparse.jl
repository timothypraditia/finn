...
breakthrough_data = c_diss[end,1:501]
profile_data = c_tot[:,501]
...
function loss_rd(p_1)
    ...
    10^3 * sum(abs2, breakthrough_data .- breakthrough_pred) + 10^2 * sum(relu.(ret[2:end] .- ret[1:end-1])) + 10^2 * abs(sum(p_1[end-2 : end-1])), pred, ret
end
...
function loss_rd(p_2)
    ...
    profile_pred = pred[Nx+1:2*Nx,end]

    # Define the loss function as a sum of the squared error
    sum(abs2, profile_data .- profile_pred), pred
end
